 ...
 *
 * Source: ESP-IDF v4.4.7 components/esp_eth/src/openeth.h
 * Vendored into WLED-MM to provide esp_eth_mac_new_openeth() which is not compiled
 * into the pre-built Tasmota/arduino-esp32 libethernet.a.
 */
...
// Provide defaults for sdkconfig values that are not available in arduino-esp32 PlatformIO builds
#ifndef CONFIG_ETH_OPENETH_DMA_RX_BUFFER_NUM
#define CONFIG_ETH_OPENETH_DMA_RX_BUFFER_NUM 4
#endif
#ifndef CONFIG_ETH_OPENETH_DMA_TX_BUFFER_NUM
...
typedef struct {
    ...
    uint16_t or_flag: 1; //!< Overrun flag (renamed from 'or' to avoid C++ keyword conflict)
